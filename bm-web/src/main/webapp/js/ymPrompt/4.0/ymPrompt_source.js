...
(function() {
	...
	(function() {
      ...
      // 20161201 Joe fix bug: iframe name cannot be dynamic setting.
      var attrs ="";
      (function (obj) {
      ...
      })(curCfg.iframe); //为iframe添加自定义属性 ,因Name无法动态设置所以需要先处理好
      ym_body.innerHTML = !curCfg.iframe ? ('<div class="' + curCfg.msgCls + '">' + curCfg.message + '</div>') : "<iframe "+attrs+" style='" + ifmStyle + "' border='0' frameborder='0' src='" + curCfg.message + "'></iframe><div style='" + ifmStyle + ";background:#000;opacity:0.1;filter:alpha(opacity=10);display:none'></div>"; //内容
			(function(el, obj) {
				for (var i in obj) try {
					el[i] = obj[i]
				} catch(e) {}
        ...
        for (var idx in btns) if(btns[idx] && btns[idx].id) rBtn.push($(btns[idx].id));
        return rBtn;
			}
		});
