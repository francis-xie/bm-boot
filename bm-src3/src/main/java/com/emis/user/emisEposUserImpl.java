 ...
 * Track+[13391] dana.gao 2009/09/02返回login user的usertype增加5.供應商7.旅行社
 * Track+[13803] dana.gao 2009/11/12 修正新版權限設定區域人員(非區域經理)無法取得區域權限問題
 * Track+[15575] dana.gao 2010/08/19 增加資料庫密碼明碼驗證,即資料庫中密碼加密,不加密均可驗證通過.
 */
public class emisEposUserImpl extends emisAbstractUser implements emisUser {
   ...
   * @throws Exception
   */
  public emisEposUserImpl(ServletContext oContext, String sSNo,
    ...
    String sCOM_NO = "";       // GL客制加入
    String sCompanyName = "";  // GL客制加入
    ...
    try {
      ...
      if (oDb.next()) {
        ...
        if (isCheckPasswd.booleanValue()) {
          ...
          if (!_sPasswd.equals(sPassWord) && !_sPWD.equals(sPassWord) ) throw new Exception("password or account error:" + sUserId_);
        }
      } else {
      ...
      sCOM_NO = this.sCompanyNo_;    // GL客制加入
      // 取得 SNo
      sSNo_ = getSNo();
      ...
      try {
        if (!"".equals(sCOM_NO)) {
          sCompanyNo_ = sCOM_NO;
          oProp_.setProperty("COM_NO", sCOM_NO);
          //System.out.print(sCOM_NO);
        }

        if (!"ZZ".equals(sCompanyNo_)) {
          sCompanyName = getCompanyName(oDb, sCompanyNo_);
          this.setAttribute("COM_NO_GL", sCompanyNo_);
          //System.out.print(sCompanyNo_);
          this.setAttribute("COM_NO_NAME", sCompanyNo_ + "  " + sCompanyName);
        } else {
          this.setAttribute("COM_NO_GL", "999999");
          this.setAttribute("COM_NO_NAME", "");
        }
...
//      if (oDb.next()) {
  ...
  private void getRegionSettings(emisDb oDb) throws SQLException {
    ...
    oDb.prepareStmt("select r.*,u.USER_TYPE from Region r with (nolock) \n" +
        " inner join RegionUser ru on r.R_NO = ru.R_NO \n" +
        " left join users u on u.ST_KEY =ru.ST_KEY  where ru.ST_KEY=? " );//+
...
//    oDb.prepareStmt("select * from Region with (nolock) where R_LEADER=? " );//+
   ...
   // oDb.setString(2, sStKey_);
    ...
    while (oDb.next()) {
      ...
      if("2".equals(oDb.getString("USER_TYPE").toString()))
...
//            + " R_LEVEL" + Integer.toString(i + 1);
  ...
  private void getUserNameFromStaff(emisDb oDb) throws SQLException {
    ...
    oDb.prepareStmt("select OP_NAME as ST_NAME from CASHIER with (nolock) where OP_NO=?");
    oDb.setString(1, sStKey_);
    oDb.prepareQuery();
...
//      }
   ...
   * @return String
   */
  public String getCompanyName(emisDb _db, String com_no) {
    String _sName = "";
    try {
      _db.prepareStmt("select COM_NAME_S from Company where COM_NO = ? ");
      _db.clearParameters();
      _db.setString(1, com_no);
      _db.prepareQuery();
      if (_db.next()) {
        _sName = _db.getString("COM_NAME_S");
      }
    } catch (Exception e) {
      e.printStackTrace();
    }
