 ...
 * $Header: /repository/src3/src/com/emis/http/emisHttpTransfer.java,v 1.1.1.1 2005/10/14 12:42:10 andy Exp $
...
public class emisHttpTransfer {
  ...
  private emisHttpFiles m_files;

  private emisHttpRequest m_formRequest;

  private PrintWriter out_;

  public static Vector UploadInstance = new Vector();


  public String sRealtimeDebug = "";

  public emisHttpTransfer() {
    ...
    m_files = new emisHttpFiles();

    m_formRequest = new emisHttpRequest();

    UploadInstance.add(this);
    
    sRealtimeDebug = "create";

  }
  
  public void remove() {
	  UploadInstance.remove(this);
  }
  ...
  public final void initialize(PageContext pageContext)
    ...
    out_ = m_response.getWriter();

    if (_DEBUG_)

      System.out.println("request=" + m_request);
  ...
  public void upload() throws emisUploadException, IOException, ServletException {
    ...
    ServletInputStream in = m_request.getInputStream();

    sCharSet = m_request.getHeader("Accept-Charset");

    m_totalBytes = m_request.getContentLength();

    if (_DEBUG_) {

      out_.println("request=" + m_request + ",in=" + in);

      out_.println("bytes=" + m_totalBytes);

      System.out.println("request=" + m_request + ",in=" + in);

      System.out.println("bytes=" + m_totalBytes);

    }

    if (m_totalBytes == 0) m_totalBytes = 1024;

    if (m_totalBytes == -1) {

      out_.println("There's no any request data.");
    ...
    //readBytes = in.read(m_binArray);  //.read(m_binArray, 0, m_totalBytes);
    ...
    for (; totalRead < m_totalBytes; totalRead += readBytes) {
      ...
      try {
        ...
        throw new emisUploadException("Unable to upload.");

      }

    }

    if(totalRead < m_totalBytes) {
    	throw new emisUploadException("data read not complete");
    }
    ...
    for (; !found && m_currentIndex < m_totalBytes; m_currentIndex++) {
      ...
      if (m_binArray[m_currentIndex] == 13)
        ...
        m_boundary = m_boundary + (char) m_binArray[m_currentIndex];  // 讀走boundsay 列

    }

    this.sRealtimeDebug = "get boundary";

    if (_DEBUG_)

      out_.println("m_currentIndex=" + m_currentIndex);  // m_currentIndex=40
    ...
    do {
      ...
      if (isFile) {
        ...
        if (sCharSet != null && !"".equals(sCharSet.trim())) {
          fileName = getFileName(URLDecoder.decode(filePathName, sCharSet));
        } else {
      ...
      if( infiniteLoop > 1000) {
    	  this.dumpFile();
    	  this.sRealtimeDebug = "infinite break"; 
    	  return;
      }
      ...
      if (isFile) {
        ...
        emisHttpFile newFile = new emisHttpFile();
        ...
        if (sCharSet != null && !"".equals(sCharSet.trim())) {
          m_formRequest.putParameter(fieldName, URLDecoder.decode(value, sCharSet));
        } else {
          m_formRequest.putParameter(fieldName, value);
        }

        //out_.println("value=" + value);
    ...
    this.sRealtimeDebug = "upload done"; 
  ...
  public int save(String destPathName)
      ...
      throws emisUploadException, IOException, ServletException {
  ...
  public emisHttpRequest getRequest() {
  ...
  public void fieldToFile(ResultSet rs, String columnName, String destFilePathName)
      ...
      throws SQLException, emisUploadException, IOException, ServletException {
    ...
    try {
      ...
      throw new emisUploadException("Unable to save file from the DataBase (1020).");
  ...
  long getDataSectionCallCnt = 0;
  
  private void getDataSection() {

	getDataSectionCallCnt++; 

	this.sRealtimeDebug = "getDataSection:" + getDataSectionCallCnt;
	...
	byte b = (byte) m_boundary.charAt(keyPos);
    ...
    do {
      ...
      if (m_binArray[searchPos] == b ) {
        ...
        keyPos++;
		b = (byte) m_boundary.charAt(keyPos);
  ...
  private String getDataHeader() {
    ...
    while (!found) {
      ...
      //out_.println("index=" + m_currentIndex);
  ...
  protected String getPhysicalPath(String filePathName, int option)
    ...
    if (option == 0) {
      ...
      if (isPhysical) {
        ...
        throw new IllegalArgumentException("This path does not exist (1135-2).");
    ...
    if (option == 1) {
      ...
      if (isPhysical)
        ...
        throw new IllegalArgumentException("This path does not exist (1135-3).");
    ...
    if (option == 2) {
      ...
      if (isVirtual(path))
        ...
        throw new IllegalArgumentException("This path does not exist (1135-1).");
  ...
  public void uploadInFile(String destFilePathName)
      ...
      throws emisUploadException, IOException {
    ...
    try {
      ...
      throw new emisUploadException("The Form cannot be saved in the specified file (1030).");
  ...
  public void dumpFile() throws IOException {
	  ...
	  FileOutputStream out = new FileOutputStream("C:\\Sas"+ (System.currentTimeMillis() % 100000)+".dat");
	  try {
		  
		  out.write(m_binArray);
		  //m_binArray
		  //m_totalBytes
	  } finally {
		  out.close();
