...
public class emisHeaderShiftIn implements emisHtHeader
{
  ...
  public void lineToData(emisStrBuf sLine) throws Exception
  {
    ...
    if( sLine.length() != 26)
      throw new Exception("Error length in ShiftIn:"+sLine.length());
    h = new head();
    h.sSNo_  = sLine.trimedsubstring(2,6);
    h.sDate_ = sLine.trimedsubstring(6,13);
    h.sToSNo_= sLine.trimedsubstring(13,17);
    h.sPackageNum_=sLine.trimedsubstring(17,19);
    h.sInDate_ = sLine.trimedsubstring(19,26);
    data_.add(h);
  }
  ...
  public void processToTmpDatabase(emisDb oDb,String sCompanyNo) throws Exception
  {
    ...
    PreparedStatement ht_header = oDb.prepareStmt("INSERT INTO HT_TMP_H (TYPE,C_NO,S_NO,HDATE,BOXNUM,SSNO,IN_DATE) VALUES (?,?,?,?,?,?,?)");
    try {
      ...
      try {
        ...
        try {
          for(int i=0;i<size;i++) {
            ...
            ht_header.setString(7,h.sInDate_);
