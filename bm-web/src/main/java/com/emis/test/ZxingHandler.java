...
public class ZxingHandler {
    ...
    public static BitMatrix GetBitMatrix(String content, int size, ErrorCorrectionLevel errorCorrectionLevel, int margin) throws WriterException {
        size = (size <= 0) ? 100 : size;
        BitMatrix bitMatrix = null;
        MultiFormatWriter multiFormatWriter = new MultiFormatWriter();
        Hashtable<EncodeHintType, Object> hints = new Hashtable<EncodeHintType, Object>();
        hints.put(EncodeHintType.CHARACTER_SET, "UTF-8");
        hints.put(EncodeHintType.MARGIN, margin); // 控制码图白边
        hints.put(EncodeHintType.ERROR_CORRECTION, errorCorrectionLevel); // 容错率
        bitMatrix = multiFormatWriter.encode(content, BarcodeFormat.QR_CODE, size, size, hints);
        return bitMatrix;
    }
     ...
     * @return BufferedImage 输出一个进过处理的BufferedImage
     */
    public static BufferedImage toBufferedImageWithAuthColor(BitMatrix matrix) {
        int H = matrix.getHeight();
        int W = matrix.getWidth();
        int L = getFinderParttenWidth(matrix) + 3;  //获取定位符边长
        int[] pixels = new int[W * H];
