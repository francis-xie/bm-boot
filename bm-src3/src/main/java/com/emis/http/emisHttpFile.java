 ...
 * $Header: /repository/src3/src/com/emis/http/emisHttpFile.java,v 1.1.1.1 2005/10/14 12:42:10 andy Exp $
...
/*
  Track+[12907] tommer.xie 2009/08/14 (v200908)針對同一門市不同人員同時上傳會失敗，其原因即為兩者上傳文件同名，現在文件命名為:"員工門市+員工編號+原有文件名"
 */


public class emisHttpFile {
  ...
  //add by tommer.xie 2009/08/14 2009/08/14 新增上傳選項，重命名上傳文件
  public void saveAs(String destFilePathName, int optionSaveAs,String sname)
    ...
    try {
      path=path.substring(0,path.lastIndexOf("\\")+1)+sname+path.substring(path.lastIndexOf("\\")+1);
      ...
      throw new emisUploadException("File can't be saved (1120).");
  ...
  public void fileToField(ResultSet rs, String columnName)
    ...
    try {
      ...
      throw new emisUploadException("Unable to save file in the DataBase (1130).");
  ...
  protected void setParent(emisHttpTransfer parent) {
